  ...
  try {
    ...
    const [
      ...
      prisma.stage.findMany({
        ...
        // Conta TODOS os leads de cada etapa (não só os OPEN), para a distribuicao
        // bater com o Pipeline: as etapas Ganho/Perdido tem leads WON/LOST e antes
        // apareciam a zero nas Análises.
...
// Responde ao filtro de período: dias para períodos curtos (today/7d/30d) e meses
// para os longos (3m/6m/1y); aceita também from/to (intervalo personalizado) e o
// legado `months`. Faz 1 query e agrupa por bucket no código (eficiente).
