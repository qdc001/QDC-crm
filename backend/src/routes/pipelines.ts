...
router.post('/', async (req: AuthRequest, res: Response, next) => {
  try {
    ...
    const pipeline = await prisma.pipeline.create({
      data: { name, description, color: color || '#3B82F6', position: count, workspaceId: req.user!.workspaceId, createdById: req.user!.id,
        stages: { create: [
...
router.patch('/:id', async (req: AuthRequest, res: Response, next) => {
  try {
    const { name, description, color } = req.body;
    const pipeline = await prisma.pipeline.update({ where: { id: req.params.id }, data: { name, description, color }, include: { stages: true } });
    res.json(pipeline);
  } catch (e) { next(e); }
});
